...
// the admin storage state is a superuser, so the "Danger zone" tab is hidden
const tabs = ["My profile", "Password", "Appearance", "API keys", "Developer"]
...
test("Tabs are deep-linkable through the ?tab search param", async ({
  page,
}) => {
  ...
  await expect(page).toHaveURL(/tab=developer/)
  await expect(
...
test("Selected mode is preserved across sessions", async ({ page }) => {
  ...
  // settings tab uses theme cards with stable ids
  await page.getByTestId("light-mode").click()
  await expect(page.locator("html")).toHaveClass(/light/)
  await page.getByTestId("dark-mode").click()
  await expect(page.locator("html")).toHaveClass(/dark/)
